...
{
    public enum GameOverAction { None, Retry, MainMenu, Exit }
    ...
    public class GameOverScreen : Screen
    {
        ...
        private readonly List<GameOverAction> _actions = new();
        ...
        public GameOverAction PendingAction { get; private set; } = GameOverAction.None;

        public GameOverScreen(SpriteFont titleFont, SpriteFont menuFont,
                              Texture2D pixel, string reason)
        {
            ...
            _items.Add("Retry");     _actions.Add(GameOverAction.Retry);
            _items.Add("Main Menu"); _actions.Add(GameOverAction.MainMenu);
            _items.Add("Exit");      _actions.Add(GameOverAction.Exit);
        ...
        private void DrawTitle(SpriteBatch sb, int sw, int sh, byte fa)
        {
            ...
            var rsz = _menuFont.MeasureString(_reason);
            float rx = (sw - rsz.X) * 0.5f;
            float ry = ty + tsz.Y + 18f;
            byte  ra = (byte)(fa * 0.55f);
            sb.DrawString(_menuFont, _reason, new Vector2(rx, ry),
                new Color(TextDim.R, TextDim.G, TextDim.B, ra));

            float lineX = (sw - 120f) * 0.5f;
            float lineY = ry + rsz.Y + 18f;
            sb.Draw(_pixel, new Rectangle((int)lineX, (int)lineY, 120, 1),
